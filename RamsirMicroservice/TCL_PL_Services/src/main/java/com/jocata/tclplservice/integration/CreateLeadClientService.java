...
@Service
public class CreateLeadClientService {
...
public ResponseEntity<?> createLead(@RequestBody CreateLeadRequest createLeadRequest){
		...
		//1. url to be called
		String createLeadUrl= "https://apicast-uat-jocatafrontendadapter.tclnprdservice.tatacapital.com/rest/jocata/v1.0/aggregatorcreateleadsfdc";
		...
		try {
			reqString = mapper.writeValueAsString(createLeadRequest);
		...
		createLeadResponse = restCaller.postForObject(createLeadUrl, reqEntity, CreateLeadResponse.class);
		
		return new ResponseEntity<CreateLeadResponse>(createLeadResponse, HttpStatus.OK);
	}
}
